 ...
 */
export const createScheduledJob = catchAsync(async (req: AuthenticatedRequest, res: Response) => {
 ...
 */
export const getUserJobs = catchAsync(async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
  if (!userId) {
    return res.status(401).json({ message: 'Unauthorized' });
  }
 ...
 */
export const cancelJob = catchAsync(async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
  if (!userId) {
    return res.status(401).json({ message: 'Unauthorized' });
  }
 ...
 * Get job statistics
 */
export const getJobStats = catchAsync(async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
  if (!userId) {
    return res.status(401).json({ message: 'Unauthorized' });
  }
